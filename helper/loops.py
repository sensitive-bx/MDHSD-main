...
def train_ssldistill(epoch, train_loader,utrain_loader, module_list, criterion_list, optimizer, opt, temp):
    """One epoch distillation"""
    ...
    criterion_div = criterion_list[1]
    criterion_kd = criterion_list[2]
    criterion_div_logit_stand = criterion_list[3]
    criterion_dist = criterion_list[4]
    ...
    for batch_idx in range(len(train_loader)):
        ...
        if opt.distill == 'kd':
            ...
            elif opt.model_s  in ['resnet8x4','wrn_40_1', 'resnet20', 'resnet32', 'resnet8', 'vgg11', 'vgg8', 'repvgg_a0']:
            ...
            if opt.model_s=='ShuffleV1':
                loss_kd = criterion_kd(f_s, f_t) * 10+ F.mse_loss(logit_tc, logit_t)
            elif opt.model_s  in ['resnet8x4','wrn_40_1', 'resnet32']:
                loss_kd = criterion_kd(f_s, f_t)+ F.mse_loss(logit_tc, logit_t)
            else:
                raise NotImplementedError
            ...
            #print("train_ssldill srd_kd_dist")
            f_s = feat_s[-2]
            if temp == 0:
                f_s = module_list[1](f_s)

            if temp == 1:
                f_s = module_list[2](f_s)
            f_t = feat_t[-2]
            ...
            loss_logit = criterion_div_logit_stand(logit_s, logit_t)
            loss_dist = criterion_dist(logit_s, logit_t)
            ...
            if temp == 1:
                f_s = module_list[2](feat_s[-1])
                f_t = module_list[3](feat_t[-1])
            loss_kd = criterion_kd(f_s, f_t)
            ...
            if temp == 1:
                factor_s = module_list[2](feat_s[-2])
                factor_t = module_list[3](feat_t[-2], is_factor=True)
            loss_kd = criterion_kd(factor_s, factor_t)
        else:
            raise NotImplementedError(opt.distill)

        #srd_kd_dist
        loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd + opt.c * loss_logit + opt.d * loss_dist
