...
def train_ssldistill(epoch, train_loader,utrain_loader, module_list, mlp_net, cos_value, criterion_list, optimizer, opt):
    """One epoch distillation"""
    global loss_dist, loss_logit
    for module in module_list:
        module.train()
    ...
    for batch_idx in range(len(train_loader)):
        ...
        if opt.distill == 'kd':
           ...
           #f_s = module_list[1](f_s)
            ...
            if opt.model_s in ['ShuffleV1', 'mobilenetv2_x0_5']:
                loss_kd = criterion_kd(f_s, f_t) * 10+ F.mse_loss(logit_tc, logit_t)
            elif opt.model_s  in ['resnet32', 'resnet8', 'repvgg_a0', 'resnet50', 'vgg8', 'vgg11', 'wrn_16_1', 'resnet20']:
                loss_kd = criterion_kd(f_s, f_t)+ F.mse_loss(logit_tc, logit_t)
            else:
                raise NotImplementedError

        # elif opt.distill == 'srd_kd':
        #     #print("train_ssldill srd_kd")
        #     f_s = feat_s[-2]
        #     #f_s = module_list[1](f_s)
        #     f_t = feat_t[-2]
        #     logit_tc = model_t(x=None, feat_s=f_s, feat_t=f_t)
        #     if opt.model_s in ['ShuffleV1', 'mobilenetv2_x0_5']:
        #         loss_kd = criterion_kd(f_s, f_t) * 10+ F.mse_loss(logit_tc, logit_t)
        #     elif opt.model_s  in ['resnet32', 'resnet8', 'repvgg_a0', 'resnet50', 'vgg8', 'vgg11', 'wrn_16_1']:
        #         loss_kd = criterion_kd(f_s, f_t)+ F.mse_loss(logit_tc, logit_t)
        #     else:
        #         raise NotImplementedError
        #
        #     loss_logit = criterion_div_logit_stand(logit_s, logit_t)
            ...
            image_weak = input_l  # 弱增强图像（标记数据）
            image_strong = input_u1  # 强增强图像（无标记数据）
            losses_dict = criterion_kd(image_weak, image_strong, target_l)
            # 获取各类损失，可以根据 MLLD 类的实际输出调整
            loss_kd = losses_dict['loss_ce'] + losses_dict['loss_kd'] + losses_dict.get('loss_cc', 0) + losses_dict.get(
                'loss_bc', 0)

        elif opt.distill == 'fam':
            # Get intermediate features from both student and teacher
            g_s = feat_s[1:-1]  # Exclude input and final layers
            g_t = feat_t[1:-1]  # Exclude input and final layers
            # Apply FAM distillation loss
            loss_kd = criterion_kd(g_s, g_t)

        elif opt.distill == 'nkd':
            # NKD需要学生和教师的logits以及目标标签
            loss_kd = criterion_kd(logit_s, logit_t, target_l[:batch_size] if batch_size > 0 else None)
